...
let last = "";
path = "include/stomplay/tag/method.hpp"
...
for (let i = 0, size = method.length; i < size; ++i) {
   ...
   s += "\tconstexpr static auto text_hash = stomplay::static_hash<" + name + ", st_method_" + name + ">::value;\n";
   s += "};\n\n";
}
s += "constexpr static auto count = " +  last + "::num + 1;\n\n";
s = nsgen(s, ["stomplay", "method", "tag"]);
s = includegen(s, ["stomptalk/method.h", "stomplay/fnv1a.hpp", "<string_view>", "<cstdint>"]);
s = guardgen(s);
...
const def_header = (elem, name, prev) => {
    ...
    s += "\tconstexpr static auto text_size = text.size();\n";
    s += "\tconstexpr static auto text_hash = stomplay::static_hash<" + name + ", st_header_" + name + ">::value;\n";
    return s;
}
...
path = "include/stomplay/tag/header.hpp"
s = "using namespace std::literals;\n\n";
last = "";
...
s = nsgen(s, ["stomplay", "header", "tag"]);
s = includegen(s, ["stomptalk/header.h", "stomplay/fnv1a.hpp", "<string_view>", "<cstdint>"]);
s = guardgen(s);
